...
def gen_xiaoqu_insert_command(info_dict):
    ...
    for il in info_list:
        if il in info_dict:
            ...
            t.append('')
    t=tuple(t)
    command=(r'insert into xiaoqu values("'+t[0]+"\","+t[1]+"\",\""+t[2]+"\",\""+t[3]+"\",\""+t[4]+"\");")
    print(command)
    return command
...
#info=re.match(r".+>(.+)</a>.+>(.+)</a>.+</span>(.+)<span>.+</span>(.+)",content)
        a = (content[content.find('<a'):])
        info = []
        info.append(re.match(r".*>(.+)<",a).groups()[0])
        b = (a[a.find('<a class', 10):])
        info.append(re.match(r".*>(.+)<",b).groups()[0])
        c = (b[b.find('/a>', 1):]).split('/')
        info.append(c[2].split('\n')[0])
        info.append(c[3].split()[0])
        for x in info:
            print("---")
            print(x)
        if info:
#info=info.groups()
            ...
            info_dict.update({u'建造时间':info[3]})
